...
# D. Roten - v2 of this function is required in order to build the column receptor grid
# 12/1/2020

config_xstilt.v2 <- function(namelist){
  ...
  output_wd <- file.path(store.path, paste('out', gsub(' ', '', site),
  ...
  # Get receptor grid information
  top.left <- as.numeric(unlist(strsplit(namelist$top.left, split = ',')))
  top.right <- as.numeric(unlist(strsplit(namelist$top.right, split = ',')))
  width <- namelist$width
  grid.res <- namelist$grid.res
  interpolation.resolution <- namelist$interpolation.resolution
  date.time <- namelist$date.time
  
  receptors <- generate.grid(top.left, top.right, width, grid.res, date.time,
                             agl = agl, interpolation.resolution,
                             output.path = output_wd)
  receptors <- subset(receptors, type == 'xstilt')[,c(1:3,6)]
