...
#' This function is identical to X-STILT's `config_xstilt()` function EXCEPT for
#' the removal of the memory argument from `slurm_options`. CHPC cannot handle
#' 
#' Further changes: added `site` to `output_wd`. And changed a minor typo:
#' `tropomi_spec` to `tropomi.spec` in an "if" statement.
#' 
#' this argument. DR 12/18/2020

config_xstilt.chpc <- function(namelist){
  ...
  output_wd <- file.path(store.path,
                         paste('out', gsub(' ', '', site),
