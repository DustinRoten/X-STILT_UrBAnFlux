...
# Turning run_xstilt into a function for repeated calling.

run_xstilt_UrBAnFlux_modeled <- function(input.variables = NULL) {
  ...
  # urbanTF    <- FALSE    # whether to search for overpasses over urban region
  ...
  # ---------------------------- Basis XSTILT flags ---------------------------- #
  run_trajec <- input.variables$run_trajec    # whether to generate trajec; T: may overwrite existing trajec
  ...
  if (oco.sensor == 'Modeled') { num.bg = NA; num.peak = NA }
  ...
  # ----------------------------- Footprint params ---------------------------- #
  foot_res  <- input.variables$foot.res  # footprint resolution in degree, 1km for ODIAC
  ...
  ak.wgt  <- FALSE              # if FALSE, AK will be set as 1 for regional sims
  pwf.wgt <- TRUE
  ...
  time_integrate <- input.variables$time_integrate  # whether to integrate footprint along time; F, hourly foot
  ...
  timeout = input.variables$timeout    # time allowed before forced terminations in second
  ...
  namelist = list(ak.wgt = ak.wgt, ct.ver = ct.ver, ctflux.path = ctflux.path, 
                  ...
                  # Variables required in config_xstilt.v2 for the generation
                  # of the receptor grid (Regional XCO2 Modeling)
                  top.left = input.variables$top.left,
                  top.right = input.variables$top.right,
                  width = input.variables$width,
                  grid.res = input.variables$receptor.resolution,
                  interpolation.resolution = input.variables$interpolation.resolution,
                  date.time = input.variables$timestamp)      
  
  cat('Done with creating namelist...\n\n')
  
  config_xstilt.v2(namelist)  # start X-STILT, either calc traj, foot or simulation
  # end of main script
  # ---------------------------------------------------------------------------- #
}